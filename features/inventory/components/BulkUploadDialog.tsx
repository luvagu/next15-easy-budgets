		...
		if (result.errors.length > 0) {
			toast.error(
				t('error_parse', {
					...
					row: String(result.errors[0].row),
				}),
			)
			return
		}
			...
			<DialogContent className='sm:max-w-3xl max-h-[90vh]'>
				...
				{step === 'paste' && (
					<div className='space-y-3'>
						...
						<div className='rounded-md border bg-muted/50 p-3'>
							...
							<p className='rounded border bg-background p-2 font-mono text-xs text-muted-foreground'>
								{AI_PROMPT}
							</p>
						</div>
						<Textarea
					...
					<div className='space-y-3 overflow-y-auto'>
							...
							<Table>
								<TableHeader>
									<TableRow>
										...
										<TableHead>{t('label_name')}</TableHead>
										<TableHead>{t('label_brand')}</TableHead>
										<TableHead>{t('label_category')}</TableHead>
										<TableHead>{t('label_cost_usd')}</TableHead>
										<TableHead>{t('col_stock')}</TableHead>
						...
						<div className='flex gap-3 text-xs text-muted-foreground'>
							<span>
								{t('label_bulk_create')}:{' '}
								{
									Object.values(resolutions).filter(r => r.action === 'create')
										.length
								}
							</span>
							<span>
				...
				{step === 'done' && (
					<div className='space-y-4 text-center py-6'>
						<CheckCircleIcon className='size-12 text-green-500 mx-auto' />
